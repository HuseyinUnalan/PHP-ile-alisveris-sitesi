...
$sliderfotosor=$db->prepare("select * from sliderfoto where slider_id=:slider_id order by sliderfoto_id");
         ...
         <div class="card">
          <div class="card-body">
            ...
            <form action="../netting/islem.php" method="POST" enctype="multipart/form-data">
              ...
              <br>
              <a class="btn btn-success" href="slider-foto-yukle.php?slider_id=<?php echo $_GET['slider_id'];?>"><i class="fa fa-plus" aria-hidden="true"></i>  Fotoğraf Yükle</a>

              <button type="submit" name="sliderfotosil"  class="btn btn-danger "><i class="fa fa-trash" aria-hidden="true"></i> Seçilenleri Sil</button>
              <?php  array("sliderfotosec"); ?>
              <div class="tab-content p-3">
                <?php 
                $sorgu=$db->prepare("select * from sliderfoto");
                $sorgu->execute();
                $sliderfotosor=$db->prepare("select * from sliderfoto where slider_id=:slider_id");
                $sliderfotosor->execute(array(
                  'slider_id' => @$_GET['slider_id']
                  ));

                  while($sliderfotocek=$sliderfotosor->fetch(PDO::FETCH_ASSOC)) { ?>
                  ...
                  <div class="tab-pane active" id="tab1">
                    ...
                    <img style="width: 300px; height: 200px; float: left; margin-right: 3px" src="../../<?php echo $sliderfotocek['sliderfoto_resimyol']; ?>" alt="image"/>
                    <hr>
                    <input type="checkbox" class="form-control"   name="sliderfotosec[]"  value="<?php echo $sliderfotocek['sliderfoto_id']; ?>">
